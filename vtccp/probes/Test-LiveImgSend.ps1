...
try {
    ...
    if ($r1 -match "Error|error|ERR") {
        Write-Host "  Device rejected LIVEIMG.MODE=2" -ForegroundColor Yellow
    } else {
    ...
    if ($rawBytes.Length -gt 3) {
        ...
        if ($rawBytes[0] -eq 0xFF -and $rawBytes[1] -eq 0xD8 -and $rawBytes[2] -eq 0xFF) {
            ...
            [System.IO.File]::WriteAllBytes($OutFile, $rawBytes)
            Write-Host "  Raw bytes saved to: $OutFile" -ForegroundColor Yellow
        }
    } else {
        Write-Host "  No data returned - command not supported at this mode." -ForegroundColor Yellow
    }
    ...
    if ($stream -ne $null) {
        Write-Host "`n[3] Restoring LIVEIMG.MODE=0"
        try {
            ...
            Write-Host "  (restore send failed)" -ForegroundColor Yellow
        }
        $stream.Close()
        $tcp.Close()
