...
function onResult(decodeResults, readerProperties, outputResults) {
    ...
    o += elem("PushScriptDiag", "v1.26 q=" + _qSrc + " m=" + (m ? "found" : "null"));
    ...
    // ── v1.26 introspection probes ────────────────────────────────────────────
    //   Retained: DebugModSize, DebugECCount (formula sanity).
    //             DebugMetricsKeys, DebugRSiblings (regression baselines).
    //             DebugSymbols0 — null for DM (v1.25 confirmed); run on
    //                            LIVE QR scan to surface QR-specific params.
    //   NEW v1.26:
    //     DebugTrucheckKeys — ES3 for-in enumeration of q (= r.trucheck)
    //                         top-level keys. Target: find where
    //                         ULQZ/URQZ/RUQZ/RLQZ and TTR/CTR sub-grades
    //                         live — X in push but A in DMST PDF.
    //     DebugMinRef       — r.metrics.extremeReflectance and
    //                         r.metrics.reflectMin shapes. v1.25
    //                         DebugMetricsKeys confirmed both keys exist;
    //                         one is the MinReflectance source.
    //   DROPPED v1.26:
    ...
    // NEW PROBE: r.trucheck (q) top-level key enumeration.
    //   v1.25 confirmed ULQZ/URQZ/RUQZ/RLQZ and TTR/CTR sub-grades are "A" in
    //   DMST PDF but "X" in push XML. The data is computed by the device — it
    //   must live in a q sub-path not yet traversed. Enumerate all top-level
    //   keys of q to find it. ES3-safe: no Object.keys(), use for-in loop.
    var _v26tkStr = "";
    if (q && typeof q === "object") {
        for (var _v26tk in q) {
            var _v26tv = q[_v26tk];
            var _v26td = (_v26tv === null) ? "null"
                : (typeof _v26tv === "object")
                    ? ((typeof _v26tv.length !== "undefined")
                        ? "[arr." + _v26tv.length + "]"
                        : "[obj]")
                    : String(_v26tv).substring(0, 20);
            _v26tkStr += _v26tk + "=" + _v26td + ";";
        }
    }
    o += elem("DebugTrucheckKeys", _v26tkStr || "(q null or empty)");

    // NEW PROBE: r.metrics.extremeReflectance and r.metrics.reflectMin shapes.
    //   v1.25 DebugMetricsKeys confirmed both keys exist on r.metrics.
    //   MinReflectance is currently empty in push XML. Per v1.19 notes,
    //   reflectanceLight / reflectanceDark were PRIMITIVES (bare numbers, not
    //   {raw,grade} objects) — these may be the same. Probe both shapes.
    var _v26er = _pick(m, "extremeReflectance");
    var _v26rm = _pick(m, "reflectMin");
    var _v26erDesc = (_v26er === null || typeof _v26er === "undefined") ? "null"
        : (typeof _v26er === "object")
            ? _enumKV22(_v26er, "extremeReflectance")
            : (typeof _v26er) + "=" + String(_v26er).substring(0, 30);
    var _v26rmDesc = (_v26rm === null || typeof _v26rm === "undefined") ? "null"
        : (typeof _v26rm === "object")
            ? _enumKV22(_v26rm, "reflectMin")
            : (typeof _v26rm) + "=" + String(_v26rm).substring(0, 30);
    o += elem("DebugMinRef", "extR:" + _v26erDesc + " reflMin:" + _v26rmDesc);
