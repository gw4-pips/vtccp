...
function onResult(decodeResults, readerProperties, outputResults) {
    ...
    // ── v1.24: promote r.symbology fields to first-class ──────────────────────
    ...
    o += elem("PushScriptDiag", "v1.24 q=" + _qSrc + " m=" + (m ? "found" : "null"));
    ...
    // ── v1.24 introspection probes (image-emission + unknowns + case fix) ────
    ...
    //   New for v1.24:
    //     • DebugImageShape   — enum r.image (capture metadata; unblocks D4).
    //     • DebugJpegProbe    — r.trucheck.jpegImage length + 80-char preview.
    //                           Full payload deferred to v1.25 pending size data.
    //     • DebugGS1          — deep enum r.validation.gs1.
    //     • DebugDodUid       — deep enum r.validation.dodUid.
    //     • DebugBarcodeAsgn  — deep enum r.barcodeAssignment (undocumented).
    //     • DebugANUCase      — case-mismatch resolver: q.axialNonuniformity
    //                           (lower-u, working) vs axialNonUniformity (upper-U,
    //                           comms-guide spelling). Reports presence of each.
    //     • DebugReaderProps  — top-level enum of readerProperties.
    //   Dropped v1.23 probes (all questions answered by filed v1.23 XML —
    //   see references/samples/live-scans/v1.23-2026-05-18-* for evidence):
    //     DebugSymbology, DebugCellDefects, DebugFPDefects, DebugDMCellDims,
    //     DebugValidation, DebugMetricShape.
    ...
    // v1.24 PROBE 1: enum r.image (capture metadata). Per comms guide expecting
    //   {index, FoV, RoI, exposureTime, gain, illEnabled, id}. If loaded-image
    //   mode injects additional keys (e.g. "source" or "filename"), they'll
    //   surface here, unblocking session-plan task D4.
    o += elem("DebugImageShape", _enumKV22(_pick(r, "image"), "r.image"));

    // v1.24 PROBE 2: r.trucheck.jpegImage length + 80-char preview. Per comms
    //   guide §3 top-line #3 this IS the verified-symbol image payload (Base64
    //   JPEG string). Pre-flight check before committing to full emission in
    //   v1.25 — a 50-300 KB base64 string would be 7-40× current XML size;
    //   need length data to decide whether Network Client / listener pipeline
    //   handles it cleanly, or whether we need chunking / a separate channel.
    var _jpg = q ? _pick(q, "jpegImage") : null;
    var _jpgLen = 0;
    var _jpgPrev = "";
    if (_jpg && typeof _jpg === "string") {
        _jpgLen = _jpg.length;
        _jpgPrev = _jpg.substring(0, 80);
    } else if (_jpg !== null && typeof _jpg !== "undefined") {
        _jpgPrev = "(non-string type=" + (typeof _jpg) + ")";
    }
    o += elem("DebugJpegProbe",
        "type=" + (typeof _jpg) + " len=" + _jpgLen +
        " preview=" + _jpgPrev);

    // v1.24 PROBE 3: deep enum r.validation.gs1. v1.23 DebugValidation showed
    //   "gs1=[obj]" but didn't drill in. GS1 validation detail is needed for
    //   the formal "Fail (Quality)" reasoning the PDF report surfaces.
    o += elem("DebugGS1",
        _enumKV22(_pick(_pick(r, "validation"), "gs1"), "r.validation.gs1"));

    // v1.24 PROBE 4: deep enum r.validation.dodUid. Same as PROBE 3 but for
    //   DoD UID validation (relevant when device is configured for that
    //   application standard).
    o += elem("DebugDodUid",
        _enumKV22(_pick(_pick(r, "validation"), "dodUid"), "r.validation.dodUid"));

    // v1.24 PROBE 5: deep enum r.barcodeAssignment. Undocumented sibling
    //   surfaced by v1.22 DebugRSiblings; comms guide doesn't enumerate it.
    //   Pure discovery — first scan with v1.24 reveals what's inside.
    o += elem("DebugBarcodeAsgn",
        _enumKV22(_pick(r, "barcodeAssignment"), "r.barcodeAssignment"));

    // v1.24 PROBE 6: ANU case-mismatch resolver. Comms guide spells the
    //   q-side metric "axialNonUniformity" (capital U); our v1.15 empirical
    //   probe found lowercase 'u' works (and v1.23 emits ANUPercent=11.5
    //   from _pick(q, "axialNonuniformity")). Resolve definitively.
    var _anuLow = q ? _pick(q, "axialNonuniformity") : null;
    var _anuUpp = q ? _pick(q, "axialNonUniformity") : null;
    o += elem("DebugANUCase",
        "lower=" + (_anuLow ? "present(grade=" + tmGrade(_anuLow) + ",raw=" + mmVal(_anuLow) + ")" : "null") +
        " upper=" + (_anuUpp ? "present(grade=" + tmGrade(_anuUpp) + ",raw=" + mmVal(_anuUpp) + ")" : "null"));

    // v1.24 PROBE 7: top-level enum of readerProperties. Looking for status3D
    //   (now wired above for FieldCalibrated/FactoryCalibrated) and any other
    //   extension props the comms guide didn't enumerate. Helps confirm
    //   v1.24's calibration-status wires reach the correct path.
    o += elem("DebugReaderProps", _enumKV22(rp, "readerProperties"));
