...
//             WIRE FIX: MinReflectance. q.minimumReflectance is a firmware
//               sentinel (raw=0/grade=F/numericGrade=0) on ALL scans —
//               confirmed by v1.27 DebugMinRef2 on DM live + QR loaded image.
//               Actual dark reflectance is q.reflectanceDark (primitive number,
//               already captured as _rdRaw for SCRlRd). Wired directly.
//             WIRE FIX: ErrorCapacityUsed. Confirmed formula: corrected × 2
//               (v1.27 DM: 7×2=14 ✓, QR: 0×2=0 ✓). Now computed from the
//               existing _ecCount loop rather than left empty.
//             STRUCTURAL LIMIT CONFIRMED (no code change needed):
//               q.topQuietZone / q.rightQuietZone = {grade:"X", numericGrade:8.8}
//               — firmware-internal compound grade on a 0–10 internal scale; NOT
//               individual ISO letter grades for ULQZ/URQZ/RUQZ/RLQZ.  These
//               remain empty in the push XML; the individual per-quadrant grades
//               are computed inside DMST and are NOT in the push channel.
//               Same for q.topTransitionRatio / rightTransitionRatio /
//               topClockTrack / rightClockTrack — all return {raw:-1, grade:X,
//               numericGrade:8.8}; per-quadrant TTR/CTR NOT accessible via push.
//             CONFIRMED (no fix possible): q.general contains exactly 7 keys
//               (verticalBWG/horizontalBWG/xDimension/contrastUniformity/
//               contrastUniformityRow/contrastUniformityCol/MRD). There is NO
//               errorCorrectionLevel, ecLevel, dataCodewords, or ecBudget key.
//               DataCodewords/ECBudget/ErrorCorrectionType(QR) remain empty or
//               fallback "QR" until A1 (DMCC guide) identifies an alternate path.
//             B7 ARRAY ELEMENT SHAPES CONFIRMED by DebugArrayElem0:
...
//               B7 writers (ModValuesSheetWriter, CwValuesSheetWriter) are now
//               structurally unblocked; pending A1 for final length confirmation.
//             NEW PROBES: DebugArrayLens (confirms codewordArray/
//               encodationAnalysisArray lengths — data vs. total codewords),
//               DebugModGradeN (confirms modulationArray element grade field
//               is literally "(" or reveals full string via index 100 probe).
//             DROPPED PROBES: DebugMinRef2, DebugQZObjs, DebugTTRCTObjs,
//               DebugGeneral, DebugArrayElem0 — all answered by v1.27 scans.
//
//   v1.27 — Wire release (2026-05-18). Driven by v1.26 live scan findings
//             (catalog: samples/live-scans/v1.26-2026-05-18-catalog.md):
//             WIRED: 8 QR-specific grade params as first-class fields.
//               DebugTrucheckKeys (v1.26) confirmed all are top-level q keys
//               (NOT in q.symbols[0] which is always null). DM returns
//               tmGrade()="" for these (sentinel {raw:-1,grade:NA} as expected).
//               ULPGrade → q.upperLeftPattern, URPGrade → q.upperRightPattern,
//               LLPGrade → q.lowerLeftPattern, HCTGrade → q.horizontalClockTrack,
//               VCTGrade → q.verticalClockTrack, ALPGrade → q.alignmentPatterns,
//               VIBGrade → q.versionInformationBlock, FIBGrade → q.formatInformationBlock.
//             BUG FIX: ErrorCorrectionType for QR. v1.26 returned "QR" (wrong;
//               should be "M"/"L"/"H"/"Q"). Try gnProp("errorCorrectionLevel")
//               (q.general sub-key) first; DebugGeneral probe will confirm.
//             NEW PROBES: DebugMinRef2, DebugQZObjs, DebugTTRCTObjs,
//               DebugGeneral, DebugArrayElem0.
//             DROPPED: DebugTrucheckKeys (full 47-key map confirmed v1.26),
//               DebugMinRef (r.metrics paths are sentinels; path is q.minimumReflectance),
//               DebugSymbols0 (always null — confirmed DM live + QR loaded v1.26).
//             Confirm by: <PushScriptDiag>v1.27 q=r.trucheck m=found</PushScriptDiag>
//
//   v1.26 — Probe release (2026-05-18). Two new probes driven by v1.25 live
//             DM scan findings (see references/samples/live-scans/v1.25-
//             2026-05-18-DM-Live-GS1Format06-catalog.md):
//             NEW: DebugTrucheckKeys — ES3 for-in enumeration of q (r.trucheck)
//               top-level keys. ULQZ/URQZ/RUQZ/RLQZ and TTR/CTR sub-grades
//               show "A" in DMST PDF but "X" in push XML — the data is on
//               the device, just at an unexplored path.  This probe reveals all
//               top-level keys so the next version can target the right sub-path.
//             NEW: DebugMinRef — probes r.metrics.extremeReflectance and
//               r.metrics.reflectMin shapes.  v1.25 DebugMetricsKeys confirmed
//               both keys exist; MinReflectance is currently empty in push XML.
//               Probe result drives the wire in v1.27.
//             RETAINED: DebugModSize, DebugECCount (formula sanity),
//               DebugMetricsKeys, DebugRSiblings (regression baselines),
//               DebugSymbols0 (null for DM — needs live QR scan to populate).
//             DROPPED (all answered by v1.25 live scan):
//               DebugPrintGrowth (raw=-1 sentinel; BWG source is q.general),
//               DebugImageShape (id/index/FoV/RoI/exposureTime/gain confirmed),
//               DebugGS1 (needs Data Format Check=GS1 device setting),
//               DebugDodUid (needs DoD UID symbol + matching standard),
//               DebugBarcodeAsgn (result=-1, unassigned).
...
function onResult(decodeResults, readerProperties, outputResults) {
    ...
    o += elem("PushScriptDiag", "v1.28 q=" + _qSrc + " m=" + (m ? "found" : "null"));
    ...
    if (q) {
        ...
        // v1.27/v1.28: ErrorCorrectionType.
        //   DM → "ECC200" (confirmed v1.26 live scan).
        //   QR → v1.27 DebugGeneral CONFIRMED q.general has exactly 7 keys:
        //        verticalBWG / horizontalBWG / xDimension / contrastUniformity /
        //        contrastUniformityRow / contrastUniformityCol / MRD.
        //        There is NO errorCorrectionLevel, ecLevel, maskPattern, or any
        //        other key.  gnProp("errorCorrectionLevel") returns "" because the
        //        key is absent from q.general entirely.  The EC level for QR is NOT
        //        in the push channel on firmware 6.1.16_sr4.  The fallback "QR"
        //        is a placeholder until A1 (DMCC guide) identifies an alternate path.
        var _ecType = _qrBranch
            ? (gnProp("errorCorrectionLevel") || gnProp("ecLevel") || ovProp("ecLevel") || "QR")
            : "ECC200";
        ...
        //   DM returns tmGrade()="" (sentinel {raw:-1,grade:NA}) for QR-only params.
        //   Loaded-image QR also returns sentinel grades (no optical measurement);
        //   a LIVE QR scan is needed to see non-sentinel values.
        o += elem("ULPGrade", tmGrade(_pick(q, "upperLeftPattern")));
        o += elem("URPGrade", tmGrade(_pick(q, "upperRightPattern")));
        o += elem("LLPGrade", tmGrade(_pick(q, "lowerLeftPattern")));
        o += elem("HCTGrade", tmGrade(_pick(q, "horizontalClockTrack")));
        o += elem("VCTGrade", tmGrade(_pick(q, "verticalClockTrack")));
        o += elem("ALPGrade", tmGrade(_pick(q, "alignmentPatterns")));
        o += elem("VIBGrade", tmGrade(_pick(q, "versionInformationBlock")));
        o += elem("FIBGrade", tmGrade(_pick(q, "formatInformationBlock")));
    ...
    // ── v1.28 introspection probes ────────────────────────────────────────────
    //   Retained: DebugModSize, DebugECCount (formula sanity).
    //             DebugMetricsKeys, DebugRSiblings (regression baselines).
    //   NEW v1.28:
    //     DebugArrayLens  — lengths of all three B7 arrays (modulationArray /
    //                       codewordArray / encodationAnalysisArray). Determines
    //                       whether codewordArray includes ECC codewords (total)
    //                       or only data codewords, and whether ea array has one
    //                       entry per encoded-unit or per data-codeword.
    //     DebugModGradeN  — full grade string of modulationArray[100] with length.
    //                       DebugArrayElem0 showed grade="(" (1 char via substring).
    //                       This probe bypasses _enumKV22 to check if "(" is the
    //                       entire string or if the full value is e.g. "(4.0)".
    //   DROPPED v1.28 (all answered by v1.27 scans):
    //     DebugMinRef2    — answered: q.minimumReflectance always sentinel;
    //                       q.reflectanceDark is MinReflectance source.
    //     DebugQZObjs     — answered: {grade:X, numericGrade:8.8} flat objects;
    //                       push-channel structural limit confirmed for ULQZ/URQZ.
    //     DebugTTRCTObjs  — answered: same pattern; TTR/CTR per-quadrant confirmed
    //                       NOT accessible via push channel.
    //     DebugGeneral    — answered: 7 keys only; no errorCorrectionLevel/dataCodewords.
    //     DebugArrayElem0 — answered: element shapes confirmed for all 3 B7 arrays.
    //   DROPPED v1.27 (carried for reference):
    //     DebugTrucheckKeys — answered: full 47-key map confirmed v1.26.
    //     DebugMinRef       — answered: r.metrics paths are sentinels.
    //     DebugSymbols0     — confirmed always null.
    //   DROPPED v1.26 (carried for reference):
    //     DebugPrintGrowth  — answered: raw=-1 sentinel; BWG is q.general.
    //     DebugImageShape   — answered: id/index/FoV/RoI/exposureTime/gain.
    //     DebugGS1          — needs Data Format Check=GS1 device setting.
    //     DebugDodUid       — needs DoD UID symbol + matching standard.
    //     DebugBarcodeAsgn  — answered: result=-1 (unassigned).
    ...
    // NEW PROBE: DebugArrayLens — lengths of all three B7 arrays.
    //   v1.27 DebugArrayElem0 confirmed element shapes but not whether
    //   codewordArray includes ECC codewords (total) or data codewords only,
    //   and whether encodationAnalysisArray length = encoded units or data CWs.
    //   Expected: DM 16×36 → cwLen=56 (32 data + 24 ECC total) or cwLen=32?
    //                        eaLen=38 (encoded chars, matches PDF) or eaLen=32?
    //             QR v3-M   → cwLen=70 (44+26) or cwLen=44?  eaLen=?
    var _v28ma  = (q) ? _pick(q, "modulationArray")         : null;
    var _v28ca  = (q) ? _pick(q, "codewordArray")           : null;
    var _v28ea  = (q) ? _pick(q, "encodationAnalysisArray") : null;
    var _v28mL  = (_v28ma && typeof _v28ma.length !== "undefined") ? _v28ma.length : -1;
    var _v28cL  = (_v28ca && typeof _v28ca.length !== "undefined") ? _v28ca.length : -1;
    var _v28eL  = (_v28ea && typeof _v28ea.length !== "undefined") ? _v28ea.length : -1;
    o += elem("DebugArrayLens",
        "modLen=" + _v28mL + ";cwLen=" + _v28cL + ";eaLen=" + _v28eL + ";");

    // NEW PROBE: DebugModGradeN — full grade string of modulationArray[100].
    //   DebugArrayElem0 (v1.27) showed grade="(" via _enumKV22 substring(0,40).
    //   Since "(" is only 1 char and 40-char limit shouldn't truncate it, the
    //   grade string appears to be literally "(". This probe bypasses _enumKV22
    //   and reads the grade field directly with explicit length reporting to
    //   confirm "(" is the complete value (not a truncation artifact).
    var _v28m100  = (_v28ma && _v28ma.length > 100) ? _v28ma[100] : null;
    var _v28gFull = (_v28m100 && typeof _v28m100["grade"] !== "undefined")
        ? String(_v28m100["grade"]) : "absent";
    o += elem("DebugModGradeN",
        "grade=" + _v28gFull + ";gradeLen=" + _v28gFull.length + ";" +
        "raw=" + (_v28m100 ? String(_v28m100["raw"]) : "?") + ";" +
        "isBlack=" + (_v28m100 ? String(_v28m100["isBlack"]) : "?") + ";");
