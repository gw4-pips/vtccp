...
//             NEW PROBES: DebugMinRef2 (q.minimumReflectance actual shape +
//               q.reflectanceDark), DebugQZObjs (q.topQuietZone/rightQuietZone
//               sub-keys → ULQZ/URQZ/RUQZ/RLQZ), DebugTTRCTObjs
//               (q.topTransitionRatio/rightTransitionRatio/topClockTrack/
//               rightClockTrack sub-keys), DebugGeneral (q.general sub-keys
//               → DataCodewords/ECBudget/ErrorCorrectionLevel/etc.),
//               DebugArrayElem0 (q.modulationArray[0]/codewordArray[0]/
//               encodationAnalysisArray[0] element shapes — B7 arrays confirmed
//               in push channel by v1.26).
...
function onResult(decodeResults, readerProperties, outputResults) {
    ...
    o += elem("PushScriptDiag", "v1.27 q=" + _qSrc + " m=" + (m ? "found" : "null"));
    ...
    if (q) {
        ...
        // v1.27: ErrorCorrectionType.
        //   DM → "ECC200" (confirmed v1.26 live scan).
        //   QR → try gnProp("errorCorrectionLevel") (q.general sub-key);
        //        v1.26 returned "QR" fallback (wrong; should be "M"/"L"/"H"/"Q").
        //        DebugGeneral probe will confirm exact key name; if it resolves,
        //        the fallback "QR" will be replaced in v1.28.
    ...
    // ── v1.27 introspection probes ────────────────────────────────────────────
    //   Retained: DebugModSize, DebugECCount (formula sanity).
    //             DebugMetricsKeys, DebugRSiblings (regression baselines).
    //   NEW v1.27:
    //     DebugMinRef2    — enumerate q.minimumReflectance shape directly +
    //                       q.reflectanceDark primitive.  MinReflectance wire
    //                       suppresses F/0 — probe reveals actual raw/grade.
    //     DebugQZObjs     — enumerate q.topQuietZone and q.rightQuietZone
    //                       sub-keys.  v1.26 confirmed these contain
    //                       ULQZ/URQZ and RUQZ/RLQZ grades; sub-key names TBD.
    //     DebugTTRCTObjs  — enumerate q.topTransitionRatio, rightTransitionRatio,
    //                       topClockTrack, rightClockTrack sub-keys (TTR/RTR/TCT/RCT).
    //     DebugGeneral    — enumerate q.general sub-keys (DataCodewords, ECBudget,
    //                       ErrorCorrectionLevel, ErrorCapacityUsed, etc.).
    //     DebugArrayElem0 — probe shape of q.modulationArray[0], codewordArray[0],
    //                       encodationAnalysisArray[0] (number vs object; B7 struct).
    //   DROPPED v1.27:
    //     DebugTrucheckKeys — answered: full 47-key map confirmed v1.26.
    //     DebugMinRef       — answered: r.metrics paths are sentinels; q.minimumReflectance is source.
    //     DebugSymbols0     — confirmed always null (DM live + QR loaded-image, v1.26).
    ...
    // NEW PROBE: DebugMinRef2 — q.minimumReflectance actual shape + reflectanceDark.
    //   v1.26 DebugMinRef confirmed r.metrics.extremeReflectance/reflectMin are both
    //   {raw:-1,grade:NA} sentinels.  Source is q.minimumReflectance (already in
    //   the MinReflectance wire at line ~930, but suppression hides grade=F/raw=0).
    //   Also emit q.reflectanceDark primitive (confirmed = 4 DM / 0 QR in v1.26).
    var _v27mr    = (q) ? _pick(q, "minimumReflectance") : null;
    var _v27mrDesc = (_v27mr === null || typeof _v27mr === "undefined") ? "null"
        : (typeof _v27mr === "object") ? _enumKV22(_v27mr, "minimumReflectance")
        : (typeof _v27mr) + "=" + String(_v27mr).substring(0, 30);
    var _v27rd = (q && typeof q["reflectanceDark"] !== "undefined")
        ? String(q["reflectanceDark"]) : "absent";
    o += elem("DebugMinRef2", "minRef:" + _v27mrDesc + " | reflDark=" + _v27rd);

    // NEW PROBE: DebugQZObjs — enumerate q.topQuietZone and q.rightQuietZone.
    //   v1.26 DebugTrucheckKeys confirmed: topQuietZone = ULQZ+URQZ container,
    //   rightQuietZone = RUQZ+RLQZ container.  Sub-key names unknown — this probe
    //   reveals the exact keys (e.g. "left"/"right" or "upper"/"lower" or grade/raw).
    var _v27tqz = (q) ? _pick(q, "topQuietZone")   : null;
    var _v27rqz = (q) ? _pick(q, "rightQuietZone") : null;
    o += elem("DebugQZObjs",
        "topQZ:" + _enumKV22(_v27tqz, "topQuietZone") +
        " | rightQZ:" + _enumKV22(_v27rqz, "rightQuietZone"));

    // NEW PROBE: DebugTTRCTObjs — enumerate the 4 compound TTR/TCT objects.
    //   v1.26 DebugTrucheckKeys confirmed all four are top-level q objects.
    //   Each contains two sub-grades (left+right halves of the row/column band).
    //   topClockTrack/rightClockTrack are DM clock tracks; for QR, horizontalClockTrack
    //   and verticalClockTrack (already wired as HCTGrade/VCTGrade) are used instead.
    var _v27ttr = (q) ? _pick(q, "topTransitionRatio")   : null;
    var _v27rtr = (q) ? _pick(q, "rightTransitionRatio") : null;
    var _v27tct = (q) ? _pick(q, "topClockTrack")        : null;
    var _v27rct = (q) ? _pick(q, "rightClockTrack")      : null;
    o += elem("DebugTTRCTObjs",
        "topTTR:"   + _enumKV22(_v27ttr, "topTransitionRatio")   +
        " | rtTTR:" + _enumKV22(_v27rtr, "rightTransitionRatio") +
        " | topCT:" + _enumKV22(_v27tct, "topClockTrack")        +
        " | rtCT:"  + _enumKV22(_v27rct, "rightClockTrack"));

    // NEW PROBE: DebugGeneral — enumerate q.general sub-keys.
    //   Expected: DataCodewords, ECBudget, ErrorCorrectionLevel, ErrorCapacityUsed,
    //   MatrixSize, BWG, MaskPattern, and other symbol-level attributes.
    //   For QR: ErrorCorrectionLevel should give "M"/"L"/"H"/"Q" for the
    //   ErrorCorrectionType wire fix.  Wire fully in v1.28 once key confirmed.
    var _v27gn = (q) ? _pick(q, "general") : null;
    o += elem("DebugGeneral", _enumKV22(_v27gn, "q.general"));

    // NEW PROBE: DebugArrayElem0 — probe B7 array element shapes.
    //   v1.26 confirmed all three arrays are directly in q (no separate DMCC needed):
    //   modulationArray=[arr.684/1369], codewordArray=[arr.56/70],
    //   encodationAnalysisArray=[arr.33/31].  This probe reveals element shape:
    //   number vs. {key:val,...} object — needed before B7 writer can be built.
    var _v27ma  = (q) ? _pick(q, "modulationArray")       : null;
    var _v27ca  = (q) ? _pick(q, "codewordArray")         : null;
    var _v27ea  = (q) ? _pick(q, "encodationAnalysisArray") : null;
    var _v27ma0 = (_v27ma && _v27ma.length > 0) ? _v27ma[0] : null;
    var _v27ca0 = (_v27ca && _v27ca.length > 0) ? _v27ca[0] : null;
    var _v27ea0 = (_v27ea && _v27ea.length > 0) ? _v27ea[0] : null;
    var _v27ma0d = (_v27ma0 === null) ? "null"
        : (typeof _v27ma0 === "object") ? _enumKV22(_v27ma0, "modArr[0]")
        : (typeof _v27ma0) + "=" + String(_v27ma0).substring(0, 20);
    var _v27ca0d = (_v27ca0 === null) ? "null"
        : (typeof _v27ca0 === "object") ? _enumKV22(_v27ca0, "cwArr[0]")
        : (typeof _v27ca0) + "=" + String(_v27ca0).substring(0, 20);
    var _v27ea0d = (_v27ea0 === null) ? "null"
        : (typeof _v27ea0 === "object") ? _enumKV22(_v27ea0, "eaArr[0]")
        : (typeof _v27ea0) + "=" + String(_v27ea0).substring(0, 20);
    o += elem("DebugArrayElem0",
        "mod[0]:" + _v27ma0d + " | cw[0]:" + _v27ca0d + " | ea[0]:" + _v27ea0d);
